//
//  TestTableViewCellReactor.swift
...
class TestTableViewCellReactor: RxBasicListItem, Reactor {
    ...
    unowned var service: RxBasicListService
    
    init(service: RxBasicListService, model: Model) {
        self.service = service
        self.initialState = State(model: model, isPush: false)
        super.init()
        identity = model.title
        cellSize = TestTableViewCell.Constraint.cellSize
    }
    
    deinit {
        print("TestTableViewCellReactor dealloc")
    }
    ...
    /// 服务列表事件转成突变
    func transformEventToMutation(event: RxBasicListService.Event) -> Observable<Mutation> {
        switch event {
