...
open class RxBasicListService {
    ...
    public enum Event {
        ...
        /// 替换组事件，参数：更新组、列表数据
        case replaceSections([SectionType], result: [SectionType])
    }
    ...
    open func fetchData(page: Int = 1) -> Observable<Result<[SectionType]>> {
    ...
    /// 替换一组或多组元素，会触发 event
    open func replace(sections: [SectionType]) -> Observable<[SectionType]> {
        return self.fetchSections()
            .flatMap({ [weak self] (oldSections) -> Observable<[SectionType]> in
                guard let strongSelf = self else { return .empty() }
                return strongSelf.saveSections(strongSelf.replace(newSections: sections, sections: oldSections))
            })
            .do(onNext: { [weak self] newSections in
                guard let strongSelf = self else { return }
                strongSelf.event.onNext(.replaceSections(sections, result: newSections))
            })
    }
    ...
    /// 列表替换组
    open func replace(newSections: [SectionType], sections: [SectionType]) -> [SectionType] {
        return newSections
    }
    /// 列表查找元素
    ...
    open func mergeSections(_ oldSetions: [SectionType], with sections: [SectionType], isLoadMore: Bool) -> [SectionType] {
        guard oldSetions.count > 0 else { return sections }
        var newSections: [SectionType] = []
        var newAddSections = sections
        oldSetions.forEach { (oldSection) in
            var didmergedOldSection: Bool = false
            for (index, newSection) in newAddSections.enumerated() {
                if oldSection.model.identity == newSection.model.identity {
                    let mergedSection = mergeSection(oldSection, with: newSection, isLoadMore: isLoadMore)
                    newSections.append(mergedSection)
                    didmergedOldSection = true
                    newAddSections.remove(at: index)
                    break
                }
            }
            if !didmergedOldSection {
                newSections.append(oldSection)
            }
        }
        newAddSections.forEach { (newAddSection) in
            newSections.append(newAddSection)
        }
        return newSections
    }
    /// 合并一组的所有元素
